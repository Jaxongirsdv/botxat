    ...
    if (len(select_lang(message.chat.id))==0):
        ...
        for u in select_lang(message.chat.id):
            if (u[0]=='uzbek'):
                ...
                buttonA1 = types.KeyboardButton("Создать писмо")
                buttonB1 = types.KeyboardButton("Отправить копию")
                markup2.add(buttonA1).add(buttonB1)
                for emp in select_employees_from_id(message.chat.id):
                    fio=emp[1]
                bot.send_message(message.chat.id, fio+ "\n Привет! Добро пожаловат на наш бот! ", reply_markup=markup2)
...
def get_text_messages(message):
    ...
    if message.text.lower() == "русский":
        ...
        keyboard.add(button)
        bot.send_message(message.chat.id, "Отправте номер телефона", reply_markup=keyboard
        )
    if message.text == "Создать писмо":
        ...
        if len(select_employees_by_id(message.chat.id))>1:
            ...
            msg=bot.send_message(message.chat.id,"Ваша Организация:",reply_markup=orgss)
            ...
            msg=bot.send_message(message.chat.id,"Кому (Организация получателя например: ООО 'Фирма')")
    ...
    if message.text == "Отправить копию":
        ...
        for regs in select_regs_by_bot_id(message.chat.id):
            regss.add("Писмо №"+regs[3]+"("+regs[1]+")")
        bot.send_message(message.chat.id,"Выберите письмо",reply_markup=regss)
    for regs in select_regs_by_bot_id(message.chat.id):
        if message.text == ("Писмо №"+regs[3]+"("+regs[1]+")"):
            bot.send_message(message.chat.id,"Выбран писмо № "+regs[3]+"\n Дата: "+regs[2]+"\n Организация(от кого): "+regs[1]+"\n Организация(кому): "+regs[4]+"\n ФИО(кому): "+regs[5]+"\n Тема: "+regs[6]+"\n Файл: "+str(regs[7]))
            #user = reg_dict[message.chat.id]
            #user.link = regs[3]
            msg=bot.send_message(message.chat.id,"Загрузите копию писмы (только в виде файл):")
    ...
    if message.text == "Nusxasini yuborish":
        ...
        regss= types.ReplyKeyboardMarkup(resize_keyboard=True,one_time_keyboard=True)
        for regs in select_regs_by_bot_id(message.chat.id):
            regss.add(regs[3]+"-xat"+"("+regs[1]+")")
        bot.send_message(message.chat.id,"Xatni tanlang",reply_markup=regss)
    for regs in select_regs_by_bot_id(message.chat.id):
        if message.text == (regs[3]+"-xat"+"("+regs[1]+")"):
            bot.send_message(message.chat.id,"Xat tanlandi № "+regs[3]+"\n Data: "+regs[2]+"\n Tashkilot(Kimdan): "+regs[1]+"\n Tashkilot(Kimga): "+regs[4]+"\n FISH(Kimga): "+regs[5]+"\n Mavzu: "+regs[6]+"\n Fayl: "+str(regs[7]))
            #user = reg_dict[message.chat.id]
            #user.link = regs[3]
            msg=bot.send_message(message.chat.id,"Xat nusxasini yuklang (faqat fayl shaklida):")
...
def process_upload(message,nomer):
    ...
    # определяем путь загрузки с именем файла
        src = 'C:/Users/fuji/Documents/GitHub/botxat/files/' + message.document.file_name
            ...
            if str(id_is_bot)=='':
                ...
                for u in select_lang(message.chat.id):
                    ...
                    if (u[0]=='русский'):
                        ...
                        for emp in select_employees(contact):
                            fio=emp[1]
                        bot.send_message(message.chat.id, fio+" \n Добро пожаловат на наш бот! ", reply_markup=markup2)
        else:
            for u in select_lang(message.chat.id):
                if (u[0]=='uzbek'):
...
def process_theme_step(message):
    try:
        ...
        nomer = add_reg_and_get_nomer(message.chat.id,user.sendorg,user.sendfio,user.theme,user.org)
        #print(nomer)
        bot.send_message(message.chat.id,"Зарегистрирован новый номер:\n № "+str(nomer) +" oт "+str(datetime.now()))
        #print("Зарегистрирован новый номер:№ ")
        markup2 = types.ReplyKeyboardMarkup(resize_keyboard=True)
        buttonB1 = types.KeyboardButton("Отправить копию")
        markup2.add(buttonB1)
        bot.send_message(message.chat.id,'Ваша Организация: ' + str(user.org) + '\n Koму: ' + str(user.sendorg) + '\n ФИО получател: ' + str(user.sendfio) + '\n Тема: ' + str(user.theme),reply_markup=markup2)
        f = open("test.docx","rb")
        bot.send_document(message.chat.id,f)
        #msg = bot.reply_to(message, 'Ведите тема писмы')
        #bot.register_next_step_handler(msg, process_theme_step)
